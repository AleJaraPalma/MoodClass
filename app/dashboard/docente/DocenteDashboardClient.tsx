...
import {
  ...
  ArrowDownCircle, ArrowUpCircle, Smile, Meh, Frown, AlertTriangle, Ticket, Lock, Sparkles
} from 'lucide-react'
...
const TIPOS_EVENTO = ['Charla', 'Taller', 'Pitch', 'Competición', 'Presentación', 'Masterclass', 'Bootcamp'] as const
...
export default function DocenteDashboardClient({
  ...
  useEffect(() => {
    async function loadCheckins() {
      ...
      if (sesionIds.length === 0) { setLoadingCheckins(false); return }
      const { data, error } = await supabase.from('checkins').select('*').in('sesion_id', sesionIds)
      if (!error && data) setCheckinsHoy(data as Checkin[])
  ...
  const [eventoForm, setEventoForm] = useState({
    nombre_evento: '', tipo_evento: 'Charla' as typeof TIPOS_EVENTO[number],
    fecha: '', hora_inicio: '09:00', hora_fin: '11:00',
    lugar: '', descripcion_evento: '',
  })
  const [eventoErrors, setEventoErrors] = useState<Record<string, string>>({})
  ...
  // Split secciones by tipo
  const seccionesRegulares = secciones.filter(s => s.tipo !== 'evento')
  const seccionesEventos = secciones.filter(s => s.tipo === 'evento')

  const seccionesSemana = seccionesRegulares.filter(s => {
  ...
  // Only regular secciones for "hoy"
  const seccionesHoy = seccionesRegulares.filter(s => s.dia_semana === todayDia)
  ...
  async function handleCerrarSinTicket(sesionId: string) {
    const { error } = await supabase
      ...
      .eq('id', sesionId)
    if (error) { toast.error('Error al cerrar la clase'); return }
  ...
  async function handleIniciarClase(seccion: Seccion) {
    ...
    if (!sesionHoy) {
      ...
      const { data: nuevaSesion, error } = await supabase
        .from('sesiones')
        .insert({
          asignatura_id: asig.id, seccion_id: seccion.id,
          fecha: today, estado: 'activa', estado_clase: 'preparada', tipo_actividad: 'Clase',
        })
        .select('*, asignaturas(nombre, codigo)')
        .single()

      if (error || !nuevaSesion) { toast.error('Error al iniciar clase'); setIniciando(null); return }
      ...
      if (error) { toast.error('Error al activar sesión'); setIniciando(null); return }
      sesionHoy = { ...sesionHoy, estado: 'activa' }
    }

    if (!sesionHoy) { toast.error('No se pudo iniciar la clase'); setIniciando(null); return }
    toast.success('¡Clase iniciada! Preparando sala en vivo...')
    router.push(`/live/${sesionHoy.id}`)
  }

  async function handleIrAlEvento(seccionId: string) {
    ...
    if (sesion) {
      ...
      toast.error('No se encontró la sesión del evento')
    }
  }
  ...
  async function handleCreateSeccion(e: React.FormEvent) {
    ...
    if (!asig) {
      const { data: newAsig, error: asigErr } = await supabase
        .from('asignaturas')
        .insert({ nombre: seccionForm.nombre_asignatura, codigo: seccionForm.codigo_asignatura, docente_id: usuario.id })
        .select()
        .single()

      if (asigErr || !newAsig) { toast.error('Error creando asignatura: ' + asigErr?.message); return }
      asig = newAsig
    ...
    if (secErr || !nuevaSeccion) { toast.error('Error creando sección: ' + secErr?.message); return }

    if (asig) {
      const { data: count, error: genErr } = await supabase.rpc('generate_sesiones_semestre', {
        p_seccion_id: nuevaSeccion.id, p_asignatura_id: asig.id,
      })
  ...
  async function handleCreateEvento(e: React.FormEvent) {
    ...
    if (!eventoForm.nombre_evento.trim()) errors.nombre_evento = 'Requerido'
    if (!eventoForm.fecha) errors.fecha = 'Requerido'
    if (!eventoForm.hora_inicio) errors.hora_inicio = 'Requerido'
    if (!eventoForm.hora_fin) errors.hora_fin = 'Requerido'
    if (!eventoForm.lugar.trim()) errors.lugar = 'Requerido'
    ...
    try {
      const ts = Date.now()
      const codigoEvento = `EVT-${ts}`
      const diaSemana = getDiaSemanaFromFecha(eventoForm.fecha)

      // Create asignatura
      const { data: asig, error: asigErr } = await supabase
        .from('asignaturas')
        .insert({
          nombre: eventoForm.nombre_evento,
          codigo: codigoEvento,
          docente_id: usuario.id,
          descripcion: eventoForm.descripcion_evento || null,
        })
        .select()
        .single()

      if (asigErr || !asig) { toast.error('Error creando evento: ' + asigErr?.message); return }

      // Create seccion with tipo='evento'
      const { data: seccion, error: secErr } = await supabase
        .from('secciones')
        .insert({
          codigo_asignatura: codigoEvento,
          nombre_asignatura: eventoForm.nombre_evento,
          sala: eventoForm.lugar,
          dia_semana: diaSemana,
          hora_inicio: `${eventoForm.hora_inicio}:00`,
          hora_fin: `${eventoForm.hora_fin}:00`,
          fecha_inicio_semestre: eventoForm.fecha,
          fecha_fin_semestre: eventoForm.fecha,
          docente_id: usuario.id,
          tipo: 'evento',
          nombre_evento: eventoForm.nombre_evento,
          tipo_evento: eventoForm.tipo_evento,
          descripcion_evento: eventoForm.descripcion_evento || null,
        })
        .select()
        .single()

      if (secErr || !seccion) { toast.error('Error creando sección del evento'); return }

      // Create the single session for this event
      const { data: sesion, error: sesErr } = await supabase
        ...
        .insert({
          ...
          tipo_actividad: eventoForm.tipo_evento,
        })
        .select()
        .single()

      if (sesErr || !sesion) { toast.error('Error creando sesión del evento'); return }

      setAsignaturas(prev => [asig, ...prev])
      setSecciones(prev => [...prev, seccion])

      setShowEventoModal(false)
      setEventoForm({
        nombre_evento: '', tipo_evento: 'Charla', fecha: '',
        hora_inicio: '09:00', hora_fin: '11:00', lugar: '', descripcion_evento: '',
      })

      toast.success(`¡Evento "${eventoForm.nombre_evento}" creado!`)
      router.push(`/live/${sesion.id}`)
    } finally {
        ...
        <div className="flex-1 overflow-y-auto px-8 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 anim-fade-up delay-1">
            <div className="card p-5 bg-white border border-slate-100 shadow-sm flex flex-col justify-between rounded-2xl">
              ...
              <div className="mt-4">
                <div className="text-2xl font-extrabold font-sora leading-none text-indigo-950">{entradas.length}</div>
            ...
            <div className="card p-5 bg-white border border-slate-100 shadow-sm flex flex-col justify-between rounded-2xl">
              ...
              <div className="mt-4">
                <div className="text-2xl font-extrabold font-sora leading-none text-indigo-950">{salidas.length}</div>
            ...
            <div className="card p-5 bg-white border border-slate-100 shadow-sm flex flex-col justify-between rounded-2xl">
              <span className="text-2xl grayscale">{getMoodEmoji(salidaMood)}</span>
              <div className="mt-4">
            ...
            <div className="mb-10 anim-fade-up delay-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {secciones.map(seccion => {
                  const esEvento = seccion.tipo === 'evento'

                  if (esEvento) {
                    return (
                      <div
                        key={seccion.id}
                        onClick={() => handleIrAlEvento(seccion.id)}
                        className="card p-5 block bg-white hover:no-underline group relative overflow-hidden border border-slate-100 cursor-pointer"
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                            style={{ background: 'linear-gradient(135deg, rgba(124,58,237,0.1), rgba(236,72,153,0.08))', border: '1px solid rgba(124,58,237,0.18)' }}>
                            <Calendar className="h-5 w-5 text-violet-600" />
                          </div>
                          <span className="badge text-[10px] font-bold"
                            style={{ background: 'rgba(124,58,237,0.1)', color: '#7C3AED', border: '1px solid rgba(124,58,237,0.2)' }}>
                            {seccion.tipo_evento || 'Evento'}
                          </span>
                        </div>
                        <h3 className="font-extrabold text-base text-indigo-950 mb-1.5 font-sora tracking-tight group-hover:text-violet-600 transition-colors line-clamp-2">
                          {seccion.nombre_evento || seccion.nombre_asignatura}
                        </h3>
                        <div className="mt-2 space-y-1.5 text-xs text-slate-500 font-medium">
                          <div className="flex items-center gap-1.5">
                            <Clock className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                            {seccion.hora_inicio.substring(0, 5)} – {seccion.hora_fin.substring(0, 5)}
                          </div>
                          {seccion.sala && (
                            <div className="flex items-center gap-1.5">
                        ...
                        <div className="mt-3 pt-3 border-t border-slate-50 flex items-center gap-1 text-xs font-bold text-violet-600 group-hover:gap-2 transition-all">
                          Ir al evento <ChevronRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1" />
                        </div>
                      </div>
                    )
                  }
                    ...
                    <div key={seccion.id} className={`card p-6 bg-white border shadow-sm flex items-start justify-between gap-4 ${isActive ? 'border-indigo-200 ring-1 ring-indigo-200/50' : 'border-slate-100'}`}>
                      ...
                      {isActive && sesion ? (
                        ...
                        <button
                          ...
                          className="btn-primary px-4 py-2.5 text-[11px] tracking-wider uppercase font-semibold flex items-center gap-1.5 shrink-0"
                        >
                          {isIniciando ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Play className="h-3.5 w-3.5" />}
          ...
          {showEventoModal && (
            <div className="modal-overlay">
              <div className="card p-8 w-full max-w-lg bg-white border border-slate-100 shadow-2xl anim-scale-in relative overflow-y-auto max-h-[90vh]">
                <button
                  onClick={() => { setShowEventoModal(false); setEventoErrors({}) }}
                ...
                <div className="mb-6">
                  ...
                  <h2 className="text-xl font-extrabold font-sora text-indigo-950">Crear Evento</h2>
                  <p className="text-xs text-slate-400 mt-1">Los participantes escanean un QR sin necesidad de registrarse.</p>
                </div>
                ...
                <form onSubmit={handleCreateEvento} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setEventoForm(f => ({ ...f, nombre_evento: e.target.value }))}
                      className={`input-field ${eventoErrors.nombre_evento ? 'border-red-400' : ''}`}
                      placeholder="Ej: Pitch Day 2026"
                    />
                    {eventoErrors.nombre_evento && <p className="text-xs text-red-500 mt-1">{eventoErrors.nombre_evento}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={e => setEventoForm(f => ({ ...f, tipo_evento: e.target.value as typeof TIPOS_EVENTO[number] }))}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className={`input-field ${eventoErrors.fecha ? 'border-red-400' : ''}`}
                      />
                      {eventoErrors.fecha && <p className="text-xs text-red-500 mt-1">{eventoErrors.fecha}</p>}
                    </div>
                    <div className="flex flex-col gap-2">
                      <div>
                        ...
                        <input
                          ...
                          onChange={e => setEventoForm(f => ({ ...f, hora_inicio: e.target.value }))}
                          className={`input-field ${eventoErrors.hora_inicio ? 'border-red-400' : ''}`}
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={e => setEventoForm(f => ({ ...f, hora_fin: e.target.value }))}
                          className={`input-field ${eventoErrors.hora_fin ? 'border-red-400' : ''}`}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setEventoForm(f => ({ ...f, lugar: e.target.value }))}
                      className={`input-field ${eventoErrors.lugar ? 'border-red-400' : ''}`}
                      placeholder="Ej: Auditorio Principal"
                    />
                    {eventoErrors.lugar && <p className="text-xs text-red-500 mt-1">{eventoErrors.lugar}</p>}
                  </div>

                  <div>
                    <label className="field-label">Descripción <span className="text-slate-400 font-normal normal-case">(opcional)</span></label>
                    <textarea
                      value={eventoForm.descripcion_evento}
                      onChange={e => setEventoForm(f => ({ ...f, descripcion_evento: e.target.value }))}
                      className="input-field"
                  ...
                  <div className="flex gap-3 pt-4 border-t border-slate-50">
                    <button
                      type="button"
                      onClick={() => { setShowEventoModal(false); setEventoErrors({}) }}
                      className="btn-secondary flex-1 py-3 text-xs uppercase tracking-wider font-semibold"
                    >
                      Cancelar
                    </button>
                    <button
                      ...
                      className="btn-primary flex-1 py-3 text-xs uppercase tracking-wider font-semibold flex items-center justify-center gap-2"
                      style={{ background: 'linear-gradient(135deg, #7C3AED, #4F46E5)', opacity: loadingEvento ? 0.75 : 1 }}
                    >
                      {loadingEvento ? <><Loader2 className="h-4 w-4 animate-spin" /> Creando...</> : <><Calendar className="h-4 w-4" /> Crear Evento</>}
                    </button>
                  </div>
                </form>
              </div>
